    ...
    def match_memory(self, qk):
        ...
        if self.temp_k is not None:
            ...
            if Tj <=8:
                ...
                # target = random.sample(list,int(mk.shape[2] *0.01))
                target = random.sample(list,int(mk.shape[2] *0.1))
                target.sort()
                mkj = mk[:,:,target]
                mkj = torch.cat([mk[:,:,0:1],mkj],2)
                mkj = torch.cat([mk[:,:,-2:-1],mkj],2)

                mkj = mkj.flatten(start_dim=2)
                mvj = mv[:,:,target]
                mvj = torch.cat([mv[:,:,0:1],mvj],2)
                mvj = torch.cat([mv[:,:,-2:-1],mvj],2)
            ...
            premem_v = premem_v.flatten(start_dim=2)
