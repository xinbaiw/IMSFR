...
from model import mod_resnet101
...
class ValueEncoderSO(nn.Module):
    def __init__(self):
        ...
        self.mask = nn.Conv2d(256, 512, kernel_size=3, padding=1)

    def forward(self, image, query, key_f16, mask):
        ...
        xpre = self.fuser(x, key_f16)

        imagequ = torch.cat([query, mask],1)

        x = self.conv1(imagequ)
        x = self.bn1(x)
        x = self.relu(x)   # 1/2, 64
        x = self.maxpool(x)  # 1/4, 64
        x = self.layer1(x)   # 1/4, 64
...
class ValueEncoder(nn.Module):
    ...
    def forward(self, image, query, key_f16, mask, other_masks):
        ...
        imagequ = torch.cat([query, mask, other_masks],1)
