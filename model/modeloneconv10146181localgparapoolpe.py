...
from model.networkoneconv10146181localgparapoolpe import STCN
...
class STCNModel:
    ...
    def do_pass(self, data, it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.para['amp']):
            ...
            if self.single_object:
                ref_v = self.STCN('encode_value', Fs[:,0], Fs[:,1], kf16[:,0], Ms[:,0])
                ref_vpe = self.STCN('encode_valuepe', Fs[:,0], Fs[:,1], kf16[:,0], Ms[:,0])

                prev_logits, prev_mask = self.STCN('segment1',
                        k16[:,:,1], kf16_thin[:,1], kf8[:,1], kf4[:,1], 
                        k16[:,:,0:1], ref_v, k16[:,:,0:1], ref_v, k16[:,:,0:1], ref_vpe)


                prev_v = self.STCN('encode_value', Fs[:,1], Fs[:,2], kf16[:,1], prev_mask)
                prev_vpe = self.STCN('encode_valuepe', Fs[:,1], Fs[:,2], kf16[:,1], prev_mask)

                values = torch.cat([ref_v, prev_v], 2)

                # Segment frame 2 with frame 0 and 1
                this_logits, this_mask = self.STCN('segment1',
                        k16[:,:,2], kf16_thin[:,2], kf8[:,2], kf4[:,2], 
                        k16[:,:,0:2], values, k16[:,:,0:1], ref_v,k16[:,:,1:2],prev_vpe)
                ...
                ref_v1 = self.STCN('encode_value', Fs[:,0], Fs[:,1], kf16[:,0], Ms[:,0], sec_Ms[:,0])
                ref_v2 = self.STCN('encode_value', Fs[:,0], Fs[:,1], kf16[:,0], sec_Ms[:,0], Ms[:,0])
                ref_v = torch.stack([ref_v1, ref_v2], 1)

                ref_v1pe = self.STCN('encode_valuepe', Fs[:,0], Fs[:,1], kf16[:,0], Ms[:,0], sec_Ms[:,0])
                ref_v2pe = self.STCN('encode_valuepe', Fs[:,0], Fs[:,1], kf16[:,0], sec_Ms[:,0], Ms[:,0])
                ref_vpe = torch.stack([ref_v1pe, ref_v2pe], 1)

                # Segment frame 1 with frame 0

                prev_logits, prev_mask = self.STCN('segment1',
                        k16[:,:,1], kf16_thin[:,1], kf8[:,1], kf4[:,1],
                        k16[:,:,0:1], ref_v,k16[:,:,0:1], ref_v, k16[:,:,0:1], ref_vpe, selector)

                prev_v1 = self.STCN('encode_value', Fs[:,1], Fs[:,2], kf16[:,1], prev_mask[:,0:1], prev_mask[:,1:2])
                prev_v2 = self.STCN('encode_value', Fs[:,1], Fs[:,2], kf16[:,1], prev_mask[:,1:2], prev_mask[:,0:1])
                prev_v = torch.stack([prev_v1, prev_v2], 1)
                values = torch.cat([ref_v, prev_v], 3)

                prev_v1pe = self.STCN('encode_valuepe', Fs[:,1], Fs[:,2], kf16[:,1], prev_mask[:,0:1], prev_mask[:,1:2])
                prev_v2pe = self.STCN('encode_valuepe', Fs[:,1], Fs[:,2], kf16[:,1], prev_mask[:,1:2], prev_mask[:,0:1])
                prev_vpe = torch.stack([prev_v1pe, prev_v2pe], 1)

                # Segment frame 2 with frame 0 and 1
                this_logits, this_mask = self.STCN('segment1',
                        k16[:,:,2], kf16_thin[:,2], kf8[:,2], kf4[:,2],
                        k16[:,:,0:2], values, k16[:,:,0:1], ref_v,k16[:,:,1:2],prev_vpe,selector)
