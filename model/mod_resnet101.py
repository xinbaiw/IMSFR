...
class ResNetmask(nn.Module):
    def __init__(self, block, layers=(2, 4, 23, 3), extra_chan=0):
        self.inplanes = 64
        super(ResNetmask, self).__init__()
        self.conv1 = nn.Conv2d(2+extra_chan, 64, kernel_size=7, stride=2, padding=3)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0])
...
def resnet18(pretrained=True, extra_chan=0):
    model = (BasicBlock, [2, 2, 2, 2], extra_chan)
    if pretrained:
...
def resnext101_32x8d(pretrained=True, progress=True, **kwargs):
    ...
    model = _resnet('resnext101_32x8d', Bottleneck1, [3, 4, 23, 3], pretrained, progress, **kwargs)
    if pretrained:
        # model.load_state_dict(model_zoo.load_url(model_urls['resnet101'], model_dir=config['nn_weights_path']))
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnext101_32x8d']))
...
def resnet101(pretrained=True, extra_chan=0):
    ...
    if pretrained:
        # model.load_state_dict(model_zoo.load_url(model_urls['resnet101'], model_dir=config['nn_weights_path']))
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet101']), extra_chan)
...
def resnet101mask(pretrained=True, extra_chan=0):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetmask(Bottleneck, [2, 4, 23, 3], extra_chan)
    if pretrained:
        # model.load_state_dict(model_zoo.load_url(model_urls['resnet101'], model_dir=config['nn_weights_path']))
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet101']), extra_chan)
